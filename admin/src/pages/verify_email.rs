...
pub fn VerifyEmailPage(
    ...
    let (verify_email, verify_result) = use_reducer_user_verify_email_then();
    ...
    use_effect(move || {
        if matches!(state(), ConnectionState::Connected(_, _)) && !submitted() {
            submitted.set(true);
            info!("SpacetimeDB connected, submitting verification token...");
            verify_email(token_clone.clone());
            clear_url();
        }
    });
            ...
            Card { class: "shadow p-4 text-center", style: "min-width: 340px; max-width: 480px;",
                match &*verify_result.read() {
                        ...
                        if matches!(state(), ConnectionState::Error) {
                            ...
                            if let Some(on_continue) = on_continue {
                                Button {
                                    ...
                                    {tid!("email-verification-continue")}
                                }
                            } else {
                                Button {
                                    color: Color::Primary,
                            ...
                            h4 { class: "mt-2 mb-1", {tid!("email-verification-verifying")} }
                            p { class: "text-muted small mb-0", {tid!("email-verification-please-wait")} }
                        }
                    },
                    Some(Ok(())) => rsx! {
                        ...
                        h4 { class: "text-success mb-2", {tid!("email-verification-success-title")} }
                        p { class: "text-muted mb-4", {tid!("email-verification-success-desc")} }
                        if let Some(on_continue) = on_continue {
                            Button {
                                color: Color::Primary,
                                class: "w-100",
                    ...
                    Some(Err(err)) => rsx! {
                        ...
                        h4 { class: "text-danger mb-2", {tid!("email-verification-failed-title")} }
                        p { class: "text-muted mb-4",
                            {tid!("email-verification-failed-desc")}
                            br {}
